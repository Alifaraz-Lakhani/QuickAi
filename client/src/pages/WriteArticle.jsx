...
const WriteArticle = () => {
  ...
  const onSubmitHandler = async (e)=>{
    ...
    try {
      setLoading(true)
      const prompt= `Write a professional ${selectedLength.length} word article about ${input}`
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <button disabled={loading} type='submit' className='w-full flex items-center justify-center gap-2 bg-gradient-to-br from-[#4a7aff] to-[#9e53ee] text-white px-4 py-2 rounded-md mt-6'>
          {
        ...
        <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96 max-h-[600px] '>
          ...
          <div className='flex-1 flex justify-center items-center'>
            <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
              ...
              <p className='text-center text-gray-500'> Enter a topic and click "Generate Article" to get started </p>
            </div>
          </div>
          ) : (
