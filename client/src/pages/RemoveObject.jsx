    ...
    const onSubmitHandler = async (e)=>{
      ...
      try {
        ...
        if(object.split(' ').length > 1){
          return toast('Please enter only single object name')          
        }
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <button disabled={loading} className='w-full flex items-center justify-center gap-2 bg-gradient-to-br from-[#417df6] to-[#8e37eb] text-white px-4 py-2 rounded-md mt-6'>
          {
          ...
          </div>
          {
            !content? (
              ...
              <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
                ...
                <p className='text-center text-gray-500'> Upload an image and click Remove Object to get started </p>
              </div>
            </div>
            ) : (
              <img src={content} alt="Processed Image" className='mt-4 rounded-md' />
            )}
